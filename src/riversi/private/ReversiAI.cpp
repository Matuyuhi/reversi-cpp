...
std::pair<int, int> ReversiAI::chooseMove()
{
    ...
    for (int start = 0; start < static_cast<int>(numTasks); start += static_cast<int>(batchSize))
    {
        const int end = static_cast<int>(std::min(numTasks, start + batchSize));
